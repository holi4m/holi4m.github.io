...
1. **intel_driver::Load()** 함수를 이용하여 취약한 드라이버를 로드 및 메타데이터를 삭제하고 해당 드라이버 핸들을 반환합니다.
    1. free 파라미터를 입력받은 경우 매핑한 드라이버를 unload합니다.
2. **kdmapper::MapDriver()** 함수를 이용하여 사용자가 전달한 드라이버를 커널에 매핑해줍니다.
...
해당 파트에서 처음 보는 **PiDDBCacheTable, KernelHashBucketList**에 대한 내용과
...
2. **intel_driver::drvier_name**을 랜덤하게 생성합니다.
...
6. **CreateFileW**를 이용하여 드라이버 핸들을 획득합니다.
    - 드라이버 이름에 Nal이 들어가는 이유는 취약한 드라이버 서비스 이름이 진짜 Nal이었습니다;;
7. **utils::GetKernelModuleAddress()** 함수를 이용해 ntoskrl.exe의 주소값을 획득합니다.
8. **intel_driver::ClearPiDDBCacheTable()** 함수를 이용해 취약한 드라이버의 **PiDDBCacheTable**을 끊어버립니다.
9. **intel_driver::ClearKernelHashBucketList()** 함수를 이용해 취약한 드라이버의 **KernelHashBucketList**를 끊어버립니다.
...
1. **NtQuerySystemInformation**을 이용하여 시스템 모듈의 크기를 가져옵니다.
2. 가져온 모듈의 크기만큼 버퍼를 할당하고, 시스템 모듈을 가져옵니다.
3. 모듈의 이름이 파라미터와 같을 모듈의 **ImageBase**를 리턴합니다.
...
해당 함수에서는 취약한 드라이버의 **PiDDBCacheTable**의 를 끊습니다.

처음 보는 커널 글로벌 변수인 **PiDDBlock(_ERESOURCE), PiDDBCacheTable(_RTL_AVL_TABLE)**과

**PiDDBCacheTable**의 List를 끊기 위한 **PiDDBCacheEntry** 구조체를 사용합니다.
...
1. 패턴을 이용하여 **PiDDBLock**과 **PiDDBCacheTable**을 참조하는 주소값을 획득합니다.
...
3. **PiDDBCacheTable**의 **TableContext** 값이 0인 경우 1로 변경합니다.
...
4. **PiDDBTable**을 수정하기위해 **PiDDBlock**을 이용해 해당 테이블의 리소스를 독점합니다.
...
5. **LookupEntry** 함수를 이용해 취약한 드라이버의 **PiDDBCacheEntry**를 획득합니다.
...
6. 취약한 드라이버의 **PiDDBCacheEntry**의 List를 끊습니다.
...
7. 이후 **PiDDBCacheTable**에서 **pFoundEntry**를 삭제하고 **PiDDBCacheTable**의 **DeleteCount**를 1 감소시키고
**TableContext**값을 다시 0으로 Set하고 리소스 독점을 해제하고 True를 리턴하면서 함수가 종료됩니다.
...
해당 함수는 **ClearKernelHashBucketList**도 위와 비슷합니다.

1. **FindPattern**으로 **g_KernelHashBucketList, g_HashCacheLock**을 찾습니다.
2. **g_HashCacheLock**을 이용하여 리소스를 독점 합니다.
3. 취약한 드라이버와 같은 이름을 가지고 있는 **HashBucketEntry**를 찾습니다.
4. 찾은 Entry의 리스트를 끊고 Free 해버립니다.
...
1. **NtQuerySystemInformation**으로 드라이버 핸들의 오브젝트를 찾습니다.
2. **Object+0x08 > Device Object+0x08 > Driver Object+0x28  > Driver Section+0x28 > Driver Name**으로 찾아가
드라이버 네임의 **length값을 0으로 변조**합니다
3. 0으로 변조하는 이유는 **MiRememberUnloadDrive**r가 드라이버 Length값이 0보다 초과하는 드라이버만 체크하기 때문입니다.
...
해당 함수는 주로 사용하지 않는 **NtAddAtom**이라는 함수를 후킹하여 해당 함수를 호출하면 매핑한 드라이버의 DriverEntry를 호출하는 작업을 수행합니다.
...
2. **MapIoSpace** 함수를 이용해 획득한 **PhysicalAddress**와 매핑되는 **VirtualAddress(가상 메모리 주소)**를 획득합니다.
3. 획득한 **VirtualAddress**를 수정합니다.
...
KDMapper에서는 모든 커널 작업을 **DeviceIoControl** 함수의 IoControlCode값을 **0x80862007**만 사용하여 수행합니다.
